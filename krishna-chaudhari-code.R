...
# data.raw (line 38) is now our matrix
...
wam_weights <- read.table("krishna-chaudhari-02-wam-output.txt", header = FALSE)
# Convert to numeric vector for easier handling
wam_weights <- as.numeric(wam_weights[,1])
cat("WAM weights loaded:", wam_weights, "\n")

# Make prediction using WAM
prediction <- sum(normalized_new_input * wam_weights[1:4])
